...
def save_poses(basedir, poses, pts3d, perm):
    ...
    for k in pts3d:
        ...
        print(f"Processing point {k}, image IDs: {pts3d[k].image_ids}")
        for ind in pts3d[k].image_ids:
            if ind - 1 >= len(cams):  # Ensure the index is within the bounds of cams
                print(f"ERROR: Index {ind} for point {k} is out of bounds for cameras array!")
                return
            if len(cams) < ind - 1:
